...
public class PyramidTopple extends ExampleBase {
    ...
    private void add_domino(Vector2f pos, boolean flipped) {
        ...
        Shape shape = (flipped ? PolyShape.createBox(body, HEIGHT, WIDTH) : PolyShape.createBox(body, WIDTH, HEIGHT));
    ...
    public Space init() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < (n - i); j++) {
                Vector2f offset = cpv((j - (n - 1 - i) * 0.5f) * 1.5f * HEIGHT, (i + 0.5f) * (HEIGHT + 2 * WIDTH) - WIDTH - 230);
